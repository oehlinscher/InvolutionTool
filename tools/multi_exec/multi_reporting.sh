...
python2 multiReportVars.py

# Prepare the different parts of the Tex-report
python2 prepareConfiguration.py
python2 prepareWaveformComparison.py waveform_comparison.tex "{%##NAME##%} & \num{%##MSIM_REL##%} & \num{%##MSIM_ABS##%} & \num{%##MSIM_SUM##%} & \num{%##GLITCHES_SPICE_MSIM##%} / \num{%##GLITCHES_MSIM##%} & \num{%##INV_REL##%} & \num{%##INV_ABS##%} & \num{%##INV_SUM##%} & \num{%##GLITCHES_SPICE_INV##%} / \num{%##GLITCHES_INV##%}\\\\"
python2 ${EXPERIMENT_SETUP_DIR}/python/prepareSchematic.py ${TARGET_FOLDER}/schematic.tex ${EXPERIMENT_SETUP_DIR}/tex/schematic.tex ${REPORT_CONFIG}
cp ${EXPERIMENT_SETUP_DIR}/tex/basic.tex ${TARGET_FOLDER}/basic.tex
python2 rankingTables.py ranking.tex "{%##NAME##%} & \num{%##VALUE##%}"
...
python2 ${EXPERIMENT_SETUP_DIR}/python/generateReportVars.py ${TARGET_FOLDER}/results.json ${TARGET_FOLDER}/variables.tex
