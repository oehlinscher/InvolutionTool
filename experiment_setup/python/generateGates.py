...
def main():
	...
	if len(sys.argv) == 5: 
		generate_gates(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], None)	
	elif len(sys.argv) == 6: 
		generate_gates(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5])	
	else:
		my_print("usage: python generateGates.py default_config_file circuit_config_file gate_dir gate_template_file required_gates", EscCodes.FAIL)
		sys.exit(1)
		
		
		
def generate_gates(default_config_file, circuit_config_file, gate_dir, gate_template_file, required_gates):
